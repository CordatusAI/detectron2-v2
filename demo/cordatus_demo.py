...
from utils.camera_detectron2 import open_cam_rtsp, open_cam_usb, open_cam_onboard
...
def get_parser():
    ...
    parser.add_argument('--usb', dest='use_usb',
                        ...
                        action='store_true')
    parser.add_argument('--csi', dest='use_csi',
                        help='use CSI cam (remember to also set --vid)',
                        action='store_true')
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        #cam = Camera(args)
        #cam.open()
        cam = open_cam_usb(args.video_dev,640,480)
        ...
        #cam = Camera(args)
        #cam.open()
        cam = open_cam_onboard(args.video_dev,640,480)
        #cam = cv2.VideoCapture(0)
        #cam.set(3, 640)
        #cam.set(4, 480)
        for vis in tqdm.tqdm(demo.run_on_video(cam)):
